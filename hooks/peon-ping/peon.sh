...
# Sets MEETING_STATUS to: SCREEN_SHARED, MIC_IN_USE, or MIC_NOT_IN_USE
# Returns 0 (true) if meeting detected, 1 (false) if not
detect_meeting() {
  MEETING_STATUS="MIC_NOT_IN_USE"
  case "$PLATFORM" in
    mac)
      # Check mic and screen sharing (CoreAudio + CGWindowList)
      local _meeting_detect
      _meeting_detect="$(find_bundled_script "meeting-detect")" || true
      if [ -n "$_meeting_detect" ] && [ -x "$_meeting_detect" ]; then
        MEETING_STATUS=$("$_meeting_detect" 2>/dev/null) || true
        [ "$MEETING_STATUS" = "MIC_IN_USE" ] || [ "$MEETING_STATUS" = "SCREEN_SHARED" ] && return 0
      fi
      return 1
      ;;
    linux)
...
for c in ['session.start','task.acknowledge','task.complete','task.error','input.required','input.required.idle','resource.limit','user.spam']:
...
if event == 'SessionStart':
    ...
    if ntype == 'permission_prompt':
        ...
        status = 'done'
        marker = '\u25cf '
        if cat_enabled.get('input.required.idle', True):
            notify = '1'
            notify_color = 'yellow'
            msg = project
...
_run_sound_and_notify() {
  ...
  # Skip desktop notifications entirely when screen is being shared
  if [ -n "$NOTIFY" ] && [ "$PAUSED" != "true" ] && [ "${DESKTOP_NOTIF:-true}" = "true" ] && [ "${MEETING_STATUS:-}" != "SCREEN_SHARED" ]; then
    [ -z "$_focused" ] && { terminal_is_focused && _focused=true || _focused=false; }
